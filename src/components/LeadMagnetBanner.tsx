...
export default function LeadMagnetBanner() {
  ...
  const enabled = pathname !== '/contacto'

  useEffect(() => {
    if (!enabled || dismissed) { setVisible(false); return }
  ...
  return (
    <div className={`nb-leadmag-banner${visible ? ' is-visible' : ''}`}>
      <button
        className="nb-leadmag-banner-close"
        aria-label="Cerrar"
      ...
      <Link className="nb-leadmag-banner-cta" href="/contacto#checklist">
        Descargar
      </Link>
    </div>
  )
